...
add_action( 'init', 'mlib_blocks_register', 5 );
add_filter( 'block_categories_all', 'mlib_block_category' );
add_filter( 'acf/blocks/no_fields_assigned_message', 'mlib_block_no_fields_msg', 10, 2 );
 ...
 * @since 0.1.1
 */
function mlib_blocks_register() {
	$blocks = mlib_get_blocks();
	...
	foreach ( $blocks as $block ) {
		if ( file_exists( MLIB_PLUGIN_BLOCKS . $block . '/block.json' ) ) {
			/**
			 ...
			 */
			register_block_type( MLIB_PLUGIN_BLOCKS . $block . '/block.json' );
		}
	}
}
 ...
 * @since 0.1.1
 */
function mlib_get_blocks() {
	// Check for options.
	$blocks  = get_option( 'mlib_blocks' );
	$version = get_option( 'mlib_blocks_version' );

	if ( empty( $blocks ) || version_compare( MLIB_VERSION, $version ) || ( function_exists( 'wp_get_environment_type' ) && 'production' !== wp_get_environment_type() ) ) {
		$blocks = scandir( MLIB_PLUGIN_BLOCKS );
		$blocks = array_values( array_diff( $blocks, array( '..', '.', '.DS_Store' ) ) );

		// Update our options.
		update_option( 'mlib_blocks', $blocks );
		update_option( 'mlib_blocks_version', MLIB_VERSION );
	}
 ...
 * @since 0.1.1
 */
function mlib_block_category( $block_categories ) {

	$block_categories = array_merge(
		array(
			array(
				'slug'  => 'mlib-blocks',
				'title' => __( 'ACF Blocks', 'mlib-blocks' ),
			),
		),
		$block_categories,
	);
 ...
 * @since 0.1.1
 */
function mlib_block_no_fields_msg( $message, $block_name ) {
	if ( 'acf/phone-number' === $block_name ) {
		$message = sprintf(
			/* translators: %s: an admin URL to the field group edit screen */
			__( 'Edit the block contentin the <a href="%s" target="_blank">Site Settings</a>.', 'mlib' ),
			admin_url( 'admin.php?page=mlib_settings' )
		);
	}
